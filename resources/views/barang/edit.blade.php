...
<div class="page-header">
  <div class="page-header-left">
    <h2>Edit Barang</h2>
    <p>Perbarui informasi barang: <strong>{{ $barang->nama_barang }}</strong></p>
  </div>
    ...
    <form method="POST" action="{{ route('barang.update',$barang) }}">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input class="form-control" name="kode_barang" value="{{ old('kode_barang',$barang->kode_barang) }}" placeholder="Contoh: OLI-10W40-01">
          @error('kode_barang')<div class="form-error" style="display:block">{{ $message }}</div>@enderror
        </div>
        <div class="form-group">
          <label class="form-label">Nama Barang *</label>
          <input class="form-control" name="nama_barang" value="{{ old('nama_barang',$barang->nama_barang) }}" required>
      ...
      <div class="form-row">
        ...
        <div class="form-group">
          ...
          <input class="form-control" name="stok" type="number" min="0" required value="{{ old('stok',$barang->stok) }}">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input class="form-control" name="stok_minimum" type="number" min="1" required value="{{ old('stok_minimum',$barang->stok_minimum ?: 5) }}">
          @error('stok_minimum')<div class="form-error" style="display:block">{{ $message }}</div>@enderror
        </div>
        <div class="form-group">
          <label class="form-label">Harga Satuan (Rp) *</label>
          <input class="form-control" name="harga_satuan" type="number" min="0" required value="{{ old('harga_satuan',$barang->harga_satuan) }}">
